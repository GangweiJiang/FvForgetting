...
def tokenize_labels(sentence_parts, text_labels, tokenizer):
    ...
    is_llama = 'llama' in tokenizer.name_or_path.lower() or 'mistral' in tokenizer.name_or_path.lower() 
...
def word_pairs_to_prompt_data(word_pairs : dict,
    ...
    if shuffle_labels:
        ...
        if prepend_space:
            ...
            prompt_data['examples'] = [{'input': w1, 'output':' ' + w2} for (w1,w2) in list(zip(*randomized_pairs))]
            prompt_data['query_target'] = {'input': str(query_target_pair['input']), 'output': ' ' + str(query_target_pair['output'])} if query_target_pair is not None else None
        ...
        if prepend_space:
            ...
            if len(str(query_target_pair['output']))==0:
                prompt_data['examples'] = [{'input': w1, 'output':str(w2)+"."} for (w1,w2) in list(zip(*word_pairs.values()))]
                prompt_data['query_target'] = {'input': str(query_target_pair['input']), 'output': str(query_target_pair['output'])+"."} if query_target_pair is not None else None
            elif str(query_target_pair['output'])[0] in ['1','2','3','4','5','6','7','8','9','0']:
                prompt_data['examples'] = [{'input': w1, 'output':str(w2)} for (w1,w2) in list(zip(*word_pairs.values()))]
                prompt_data['query_target'] = {'input': str(query_target_pair['input']), 'output': str(query_target_pair['output'])} if query_target_pair is not None else None
            else:
                prompt_data['examples'] = [{'input': w1, 'output':' ' + str(w2)} for (w1,w2) in list(zip(*word_pairs.values()))]
            ...
            # prompt_data['examples'] = [{'input':w1[:-1], 'output':' ' + str(w2)} for (w1,w2) in list(zip(*word_pairs.values()))]
            # prompt_data['query_target'] = {'input': str(query_target_pair['input'])[:-1], 'output':' ' +  str(query_target_pair['output'])} if query_target_pair is not None else None
...
def split_icl_dataset(dataset, train_size=None, test_size=0.3, seed=42) -> Dict[str,ICLDataset]:
    ...
    if test_size != 1:
        ...
        test, valid = train_test_split(valid, test_size=0.5, random_state=seed)
    else:
        train = dataset.raw_data[-50:]
        test = dataset.raw_data[-200:]
        valid = dataset.raw_data
