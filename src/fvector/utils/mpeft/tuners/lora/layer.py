...
class LoraLayer(BaseTunerLayer):
    ...
    def update_layer(
        self, adapter_name, r, lora_alpha, lora_dropout, init_lora_weights, use_rslora, use_dora: bool = False, r_sum=0
    ):
    ...
    def reset_lora_parameters(self, adapter_name, init_lora_weights):
        ...
        if adapter_name in self.loranew_A.keys():
            if init_lora_weights is True:
                ...
                nn.init.normal_(self.loranew_A[adapter_name].weight, std=1 / self.r[adapter_name])
            else:
    ...
    def forward(self, x: torch.Tensor, *args: Any, **kwargs: Any) -> torch.Tensor:
        ...
        if self.disable_adapters:
            ...
            for active_adapter in self.active_adapters:
                ...
                result += (
                    self.loranew_B[active_adapter](
                        self.loranew_A[active_adapter](dropout(x))
                    ) * scaling
                )
...
def dispatch_default(
    ...
    **kwargs,
    ...
    if isinstance(target_base_layer, torch.nn.Embedding):
        ...
        new_module = Linear(target, adapter_name, in_features=in_features, out_features=out_features, bias=bias, r_sum=lora_config.r_sum, **kwargs)
